USE workflow;
UPDATE D001_FormulariosData AS F,
  (SELECT
    FD.D001_FormulariosDataID as id,
    CONCAT(
     ...
     AND ( FD.D001_FormulariosFieldsID IN (16, 48) AND OW.DataAbetura IS NOT NULL )
     AND DATE(FD.Created_at) >= '2021-01-01'
     ORDER BY FD.Created_at DESC) AS src
     
SET F.Descricao = src.nova_data
WHERE F.D001_FormulariosDataID = id;